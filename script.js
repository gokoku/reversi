...
function init() {
  ...
  for (let i = 0; i < boardSize.y; i++) {
    board[i] = []
    for (let j = 0; j < boardSize.x; j++) board[i][j] = diskColor.none
  }
...
function takeTurn() {
  if (turn === diskColor.dark) turn = diskColor.light
  else turn = diskColor.dark
}
...
function checkCanPlace(color, pos, reverse) {
  let result = false
  if (board[pos.y][pos.x] !== diskColor.none) return false
    ...
    for (let j = -1; j <= 1; j++) {
      ...
      while (true) {
        ...
        if (board[checkPos.y][checkPos.x] === color) {
          ...
          if (reverse) {
            console.log(`${checkPos.y} : ${checkPos.x}`)
...
function draw() {
  if (!isGameEnd()) {
    ...
    message += '<br>矢印カーソル移動<br>[その他のキー]:石を置く'
  } else {
    message += '何かキーを押して下さい。'
  }
  ...
  html = ''
  for (let i = 0; i < boardSize.y; i++) {
    for (let j = 0; j < boardSize.x; j++) html += diskAA[board[i][j]]
    if (i === cursorPos.y) html += '←'

    html += '<br>'
  }

  for (let i = 0; i < boardSize.x; i++) html += i === cursorPos.x ? '↑' : '　'
  html += '<br>'
